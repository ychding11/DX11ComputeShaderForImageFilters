...
void PSMandelbrot( out vec4 fragColor, in vec2 fragCoord )
{
    float2 resolution = float2(cWidth,cHeight);
	float2 st = input.Pos.xy / cHeight; // convert pixel coordinate to [0,1)
    ...
    // animation	
	float tz = 0.5 - 0.5*cos(0.225*cTime); // can be controled
    float zoo = pow( 0.5, 6.0*tz );
	float2 c = float2(-0.05,.6805) + p*zoo; //should be control
